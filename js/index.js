...
}(function ($, window, document)
{
	...
	function getArtistImage(artistAPIResp)
	{
		...
		let artistName = artistAPIResp.artist;

		//If an artist does not exist, append an "image unavailable" image		
		if (!artistName)
		{
		...
		else
		{
			...
			fetch(imageURL)
				...
				.then(imageData =>
				{
					...
					//For cases where artists have a WIKI entry but no profile image available
	...
	function displayResults(artistTrackAPIResponse, artistAPIResponse)
	{
		...
		else if (artistAPIResponse.artist.similar.artist.length === 0)
		{
			...
			//format play count numbers to add commas		
	...
	//loop over API data to get top 5 tracks & similar artists
